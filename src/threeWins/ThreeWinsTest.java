...
class ThreeWinsTest {
    ...
    @ParameterizedTest
    @MethodSource
    void givenValidParamsButChoosenColumnIsFull_whenPlaceGamePiece_returnFalse(String[][] matchfield, int player, int column) {
        threeWins.setMatchfield(matchfield);
        assertEquals(false, threeWins.placeGamePiece(player, column));
    }

    static Stream<Arguments> givenValidParamsButChoosenColumnIsFull_whenPlaceGamePiece_returnFalse() {
        return Stream.of(
                Arguments.of(new String[][]{
                        {"O", " ", " ", " "},
                        {"X", " ", " ", " "},
